...
const conversationEntries = [
  ...
  },
  {
    ...
    response:
      "Srivatsankith is a fourth-year B.Tech student in Information Science and Engineering at M S Ramaiah University of Applied Sciences, Bengaluru. He focuses on AI, Machine Learning, and building practical intelligent systems through hands-on projects.",
    spokenText:
      "Srivatsankith is a fourth year B Tech student in Information Science and Engineering at M S Ramaiah University of Applied Sciences, Bengaluru. He focuses on artificial intelligence, machine learning, and building practical intelligent systems through hands on projects."
  },
  {
    id: "education",
    section: "experience",
  ...
  },
  {
    ...
    response: "His current CGPA is 8.25.",
    spokenText: "His current C G P A is eight point two five."
  },
  {
    id: "skills",
    section: "skills",
...
function setProfileInitials(name) {
  const profileImageWrapper = document.querySelector(".hero-profile-image");
  ...
  // Set initials as a data attribute for the ::before pseudo-element to display
  if (initials) {
    profileImageWrapper.dataset.initials = initials;
  }
}
...
function resolveMediaUrl(url) {
  ...
  // For Base64 data URLs or absolute URLs, return them as is.
  if (/^(data:image|https?:\/\/)/i.test(url)) {
    return url;
  }
  // For relative paths (like /uploads/...), return them as is.
  // The browser will resolve them relative to the current domain.
  return url;
}

function getResumeEmbedUrl(url) {
  // This function is simplified. It now expects a direct link to a PDF.
  // The complex logic for handling various Google Drive links has been removed
  // as it's unreliable and was pointing to a local file path.
  // Simply return the URL provided in `externalLinks.resume`.
  return url || "";
}
...
function openResumeModal() {
  ...
  viewer.src = getResumeEmbedUrl(externalLinks.resume);
...
function renderProfileData(profileItems) {
  ...
  const defaultName = "T Srivatsankith";
  const currentName = profileName?.textContent?.trim() || defaultName;

  // Find the first profile item that has a description (image URL)
  const profile = profileItems.find((item) => item.description) || profileItems[0] || null;

  const imageUrl = profile ? resolveMediaUrl(profile.description) : null;
  ...
  if (imageUrl && profileImage) {
    ...
    profileImage.onerror = () => {
      console.warn(`Failed to load profile image from: ${imageUrl}`);
      profileImageWrapper?.classList.remove("has-photo");
      setProfileInitials(currentName); // Fallback to initials on error
    };
    profileImage.src = imageUrl;
  } else {
    // No image URL from backend, show initials
    profileImageWrapper?.classList.remove("has-photo");
    setProfileInitials(currentName);
  }
}
...
function renderProjects(allProjects) {
  ...
  if (!gridContainer || !filterContainer || !allProjects) {
...
document.querySelectorAll("[data-open-resume-modal]").forEach((trigger) => {
...
document.querySelector(".close-modal-btn")?.addEventListener("click", closeResumeModal);
...
function initRoleTypingAnimation() {
...
async function loadHomepageData() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Homepage data fetch failed: ${response.statusText}`);
    }
    const data = await response.json();

    renderProfileData(data.profile || []);
    renderProjects(data.projects || []);
    renderHackathonItems(data.hackathons || []);
    renderInternshipItems(data.workExperience || []);
    renderActivityItems(data.activities || []);

    // All content is loaded and rendered, remove skeleton loaders
...
document.addEventListener("DOMContentLoaded", () => {
  // Initialize the futuristic background animation.
  // This looks for <div id="neural-network-container"></div> in the main HTML file
  // and replaces its content with the new canvas background.
  const neuralContainer = document.getElementById("neural-network-container");
  if (neuralContainer && typeof initFuturisticBackground === "function") {
    initFuturisticBackground(neuralContainer);
  }
  initRoleTypingAnimation();
});
